...
interface ProtocolCountCardProps {
...
export default function ProtocolCountCard({
  data,
  isLoading,
  isSelected,
  onClick
}: ProtocolCountCardProps) {
  if (isLoading) {
    return (
    ...
    >
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          {/* Protocol density indicator */}
          <div className="flex items-center space-x-1 mt-2">
            {Array.from({ length: Math.min(5, Math.floor(data.value / 10)) }, (_, i) => (
              <div key={i} className="w-2 h-2 bg-orange-500 rounded-full" />
            ))}
            {Array.from({ length: Math.max(0, 5 - Math.floor(data.value / 10)) }, (_, i) => (
